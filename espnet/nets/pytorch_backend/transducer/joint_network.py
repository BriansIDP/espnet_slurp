...
class JointNetwork(torch.nn.Module):
    ...
    def forward(
        ...
        if is_aux:
            ...
            if self.useKB and h_plm is not None:
                z = self.joint_activation(self.lin_enc(h_enc) + self.lin_dec(h_dec) + self.lin_plm(h_plm))
            else:
                z = self.joint_activation(self.lin_enc(h_enc) + self.lin_dec(h_dec))
        output = self.lin_out(z)
