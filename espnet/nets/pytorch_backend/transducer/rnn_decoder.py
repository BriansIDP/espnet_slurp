...
class DecoderRNNT(TransducerDecoderInterface, torch.nn.Module):
    ...
    def rnn_forward_batch(self, ys, states):
        """RNN forward.

        Args:
            ys (torch.Tensor): batch of input features (B, 1)
            state (tuple): batch of decoder states
                ((L, B, dec_dim), (L, B, dec_dim))
        ...
        y = self.dropout_embed(self.embed(ys))
        h_prev, c_prev = states
        h_next, c_next = self.init_state(ys.size(0))
