    ...
    def recognize(self, x, recog_args, char_list=None, rnnlm=None, use_jit=False):
        ...
        if rnnlm:
            hyp = {
                ...
                "yseq": [y],
                "rnnlm_prev": None,
                "a_accum": torch.zeros(h.size(0), dtype=torch.float32),
                "final_score": 0.0
            }
        else:
            hyp = {
        ...
        for i in six.moves.range(maxlen):
            ...
            for hyp in hyps:
                ...
                cur_att_w = self.decoder.decoders[0].src_attn.attn[0, :, -1, :]
                merged_cur_att_w = cur_att_w.mean(dim=0)[0]
                hyp["a_accum"] += merged_cur_att_w
                coverage_score = hyp["a_accum"].clip(0.001, 0.5).log().sum()
                local_att_scores += recog_args.coverage_penalty * coverage_score
