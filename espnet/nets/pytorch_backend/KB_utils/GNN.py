...
class GCNSage(torch.nn.Module):
    def __init__(self, embdim, treehid, nlayer, dropout, residual=False, tied=False, nhead=1,
                bilinear=False, attn_dim=1, gcnlayer=2):
        ...
        for i in range(self.gcnlayer - 1):
            # GCN layers
            setattr(self, 'gcn_l{}'.format(i+2), torch.nn.Linear(self.treehid, self.treehid))
            if self.tied and i < self.gcnlayer - 2:
                getattr(self, 'gcn_l{}'.format(i+2)).weight = self.gcn_l1.weight
                getattr(self, 'gcn_l{}'.format(i+2)).bias = self.gcn_l1.bias
        ...
        self.permute_layer_1 = torch.nn.Linear(self.treehid, attn_dim)
        self.permute_layer_2 = torch.nn.Linear(self.treehid, attn_dim)
        if self.bilinear:
            self.bilinear_proj = torch.nn.Linear(attn_dim, attn_dim)
        self.dropout = torch.nn.Dropout(dropout)
        self.nlayer = nlayer
    ...
    def forward(self, prefixtree, decemb):
        ...
        # Forward GCN
        gcn_nodes_encs = self.dropout(self.forward_gcn(embeddings, adjacency))
        # Forward Sage
        sage_nodes_encs = self.dropout(self.forward_sage(embeddings, adjacency))
        # Combine both
        gcn_nodes_encs = self.permute_layer_1(gcn_nodes_encs)
        sage_nodes_encs = self.permute_layer_2(sage_nodes_encs)
        if self.nhead >= 1:
            nodes_encs = torch.cat([gcn_nodes_encs, sage_nodes_encs], dim=-1)
        else:
            if self.bilinear:
                gcn_nodes_encs = torch.tanh(gcn_nodes_encs)
                sage_nodes_encs = torch.tanh(sage_nodes_encs)
                nodes_encs = self.bilinear_proj(gcn_nodes_encs * sage_nodes_encs)
                # Use residual connection
                nodes_encs = nodes_encs + gcn_nodes_encs + sage_nodes_encs
            else:
                nodes_encs = torch.stack([gcn_nodes_encs, sage_nodes_encs], dim=1)
                nodes_encs = nodes_encs.max(dim=1)[0]
