...
class GAT(torch.nn.Module):
    ...
    def __init__(self, num_of_layers, num_heads_per_layer, num_features_per_layer, add_skip_connection=True, bias=True,
                 dropout=0.6, layer_type=LayerType.IMP2, log_attention_weights=False, boost_self=0):
        super().__init__()
...
class GATLayer(torch.nn.Module):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, layer_type, concat=True, activation=nn.ELU(),
                 dropout_prob=0.6, add_skip_connection=True, bias=True, log_attention_weights=False, boost_self=0):
        ...
        self.boost_self = boost_self
        if boost_self:
            self.selfproj = nn.Linear(num_in_features, num_of_heads * num_out_features, bias=False)
...
class GATLayerImp2(GATLayer):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, concat=True, activation=nn.ELU(),
        ...
        super().__init__(num_in_features, num_out_features, num_of_heads, LayerType.IMP2, concat, activation, dropout_prob,
                         add_skip_connection, bias, log_attention_weights, boost_self=boost_self)
    ...
    def forward(self, data):
        ...
        if self.boost_self > 0:
            scaling_mask = 1 / connectivity_mask.sum(dim=1, keepdim=True).unsqueeze(-1) * self.boost_self
            selfboost = self.selfproj(in_nodes_features).view(-1, self.num_of_heads, self.num_out_features)
        ...
        all_attention_coefficients = self.softmax(all_scores.masked_fill((1-connectivity_mask).bool(), -1e9))
        ...
        if self.boost_self > 0:
            out_nodes_features = out_nodes_features * (1 - scaling_mask) + selfboost * scaling_mask
        #
