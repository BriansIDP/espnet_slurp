...
def set_weight_noise_on():
    """Extension to turn on weight decay.
    ...
    @training.make_extension(trigger=(1, "epoch"))
    def set_weight_noise_on(trainer):
        _set_weight_noise_on(trainer)

    return set_weight_noise_on


def _set_weight_noise_on(trainer):
    if hasattr(trainer.updater, "weight_noise_on"):
        if not trainer.updater.weight_noise_on:
            trainer.updater.weight_noise_on = True
            logging.warning("Weight noise is now turned ON as model stops improving")
    else:
        logging.warning("Updater has no attribute `weight_noise_on`")
...
def _torch_snapshot_object(trainer, target, filename, savefun):
    ...
    s.save(trainer)
    if hasattr(trainer.updater.model, "model") and not hasattr(trainer.updater.model, "shadow"):
        # (for TTS)
...
def torch_resume(snapshot_path, trainer, strict=True):
...
def parse_hypothesis(hyp, char_list):
    ...
    # final_score = float(hyp["final_score"])
