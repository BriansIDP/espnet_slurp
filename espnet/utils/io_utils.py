...
def get_meetings(uttid_list):
    if len(uttid_list) > 0 and len(uttid_list[0].split('_')) > 3: # for AMI
        return [uttid.split('_')[1] for uttid in uttid_list]
    elif len(uttid_list) > 0 and '_' not in uttid_list[0]: # for Librispeech
        return [uttid.split('-')[1] for uttid in uttid_list]
    else: # for SWB
        return [uttid.split('-')[0] for uttid in uttid_list]
...
class LoadInputsAndTargets(object):
    ...
    def _create_batch_asr(self, x_feats_dict, y_feats_dict, uttid_list, utt_text=[],
        ...
        if self.load_output:
            ...
            nonzero_idx = list(filter(lambda i:
                (len(ys[0][i]) > 0) and (len(xs[0][i]) / len(ys[0][i]) >= self.min_ratio)
                and (len(xs[0][i]) / len(ys[0][i]) <= self.max_ratio), range(len(ys[0]))))
            for n in range(1, len(y_feats_dict)):
                nonzero_idx = filter(lambda i:
                    (len(ys[n][i]) > 0) and (len(xs[0]) / len(ys[n][i] >= self.min_ratio))
                    and (len(xs[0][i]) / len(ys[n][i]) <= self.max_ratio), nonzero_idx)
        else:
