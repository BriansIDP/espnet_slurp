...
with open("../data/Biasing/Librispeech_blists/all_rare_words.txt") as fin:
    rareset = set()
    for line in fin:
...
print('=' * 89)
print('Common words error rate: {} / {} = {}'.format(
    common_error + insert_common, common_freq, (common_error + insert_common) / common_freq))
print('Rare words error rate: {} / {} = {}'.format(
    rare_error + insert_rare, rare_freq, (rare_error + insert_rare) / rare_freq))
print('OOV words error rate: {} / {} = {}'.format(oov_error, oov_freq, oov_error / max(oov_freq, 1)))
