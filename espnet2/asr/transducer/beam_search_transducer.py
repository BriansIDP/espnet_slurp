...
class BeamSearchTransducer:
    ...
    def default_beam_search(self, enc_out: torch.Tensor, lextree: list = None) -> List[Hypothesis]:
        ...
        kept_hyps = [Hypothesis(score=0.0, yseq=[self.blank_id], dec_state=dec_state, lextree=lextree)]
        cache = {}
        cache_lm = {}
        ...
        for enc_out_t in enc_out:
            ...
            while True:
                ...
                # biasing
                if self.biasing and lextree is not None:
                    p_gen = torch.sigmoid(self.pointer_gate(torch.cat([joint_act, hptr], dim=-1)))
    ...
    def get_step_biasing_embs(self, char_ids, trees, origTries, node_encs=None):
        ...
        maxlen += 1
        step_mask = []
        back_transform = torch.zeros(len(new_trees), maxlen, ooKB_id+1, device=self.decoder.device)
